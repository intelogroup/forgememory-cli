...
## Corpus Check
- 141 files · ~129,697 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1782 nodes · 4176 edges · 96 communities (73 shown, 23 thin omitted)
- Extraction: 81% EXTRACTED · 19% INFERRED · 0% AMBIGUOUS · INFERRED: 791 edges (avg confidence: 0.8)
...
Cohesion: 0.07
Nodes (88): acquireDistillLock(), cleanDistillLock(), distillLockPath(), isDistillLockStale(), Config, DB, Time, hasExplicitInferenceProviderConfig() (+80 more)

### Community 1 - "daemon.go"
Cohesion: 0.06
Nodes (87): acquireDaemonLock(), acquirePIDLock(), acquireStartupLock(), cleanAddr(), cleanLock(), cleanPID(), cleanSocket(), cleanStartupLock() (+79 more)
...
### Community 4 - "hook.go"
Cohesion: 0.06
Nodes (75): ClaudeHookInput, buildSessionRecallOutput(), checkpointKey(), collectPromptCandidates(), compactWhitespace(), confidenceLabel(), displayProjectName(), envInt() (+67 more)
...
Cohesion: 0.06
Nodes (60): T, resolveGoEnv(), TestRunConfig_AutoValidatesAndRejectsBadKey(), TestRunConfig_NoValidateSkipsNetwork(), T, TestRunConfig_Antigravity(), TestRunConfig_DefaultModelAnthropicHaiku(), TestRunConfig_DistillBatchSize() (+52 more)
...
Cohesion: 0.21
Nodes (21): buildWebFailureQuery(), buildWebPromptQuery(), webSearchSource(), buildFailureQuery(), buildPrincipleSearchTerms(), buildQuery(), compactQuery(), dedupeKey() (+13 more)
...
### Community 18 - "DB"
Cohesion: 0.06
Nodes (33): Event, Principle, ProjectTimelineEntry, testError, DB, buildFTSQuery(), decodeStringSlice(), encodeStringSlice() (+25 more)
...
Cohesion: 0.05
Nodes (77): Buffer, Context, ExternalContextSummary, RetrievalJob, DB, Rows, scanExternalContextSummaries(), aiSummaryKind() (+69 more)
...
Cohesion: 0.16
Nodes (12): runDashboard(), defaultPath(), DB, Open(), OpenReadOnly(), readonlyDBError(), T, TestDistillationHealth_RecordSuccessAndFailure() (+4 more)

### Community 24 - "worker_test.go"
Cohesion: 0.17
Nodes (21): DB, NewWorker(), context7SetupTestServer(), Reader, Server, T, Writer, readMCPTestMessage() (+13 more)
...
Cohesion: 0.25
Nodes (16): EnqueuePromptRetrieval(), T, TestBuildWebQuery_AddsBestPracticeForPrompt(), TestBuildWebQuery_AddsYearAndFixForFailure(), TestBuildWebQuery_StripsContext7Boilerplate(), TestEnqueueFailureRetrieval_QueuesContext7ForRepeatedRustFailure(), TestEnqueueFailureRetrieval_QueuesWebSearchWhenExaKeySet(), TestEnqueueFailureRetrieval_SkipsExternalWhenRelevantPrincipleExists() (+8 more)

### Community 27 - "context7MCPClient"
Cohesion: 0.15
Nodes (12): 🔒 ForgeMemo CLI — Hardening Roadmap, P0 — Injection-Time Integrity Check, P0 — Key Rotation & Compromise Recovery, P0 — Principles at Rest: Sign or MAC Every Stored Principle, P1 — Escalation Path Audit, P1 — Event Ingestion Integrity (Distillation Pipeline), P1 — Immutable Event Log / Append-Only Table, P1 — Unix Socket Authentication (+4 more)

### Community 28 - "DB"
Cohesion: 0.36
Nodes (9): T, TestDistillationProducesPrinciples(), TestEventMetadataCorrect(), TestFTS5SearchOptimized(), TestHookToDaemonQueue(), TestInsertPrincipleDuplicateIgnored(), TestUndistilledEventsExcludesDistilled(), TestWALConcurrentWrites() (+1 more)
...
Cohesion: 0.12
Nodes (15): [0.4.10] - 2026-04-04, [0.5.14] - 2026-07-01, [0.5.6] - 2026-06-20, [0.5.9] - 2026-06-30, [0.6.1] - 2026-07-01, [0.6.3] - 2026-07-10, [0.6.4] - 2026-07-14, Changed (+7 more)
...
Cohesion: 0.67
Nodes (3): T, TestFreshExternalContextSummariesByProjectSortsByLastUsedAt(), TestTouchExternalContextSummariesUpdatesLastUsedAt()
...
- **Why does `Open()` connect `.fetchExaSearch` to `captureStdout`, `daemon.go`, `main.go`, `Open`, `hook.go`, `integration_test.go`, `distill_test.go`, `server_test.go`, `init_integrations.go`, `failures.go`, `DB`, `ai_history.go`, `worker_test.go`, `principles_test.go`, `DB`, `worker.go`, `RetrievalJob`, `openAlertsDB`, `cross_session_test.go`, `[0.6.3] - 2026-07-10`?**
  _High betweenness centrality (0.321) - this node is a cross-community bridge._
- **Why does `runDoctor()` connect `daemon.go` to `captureStdout`, `runConfig`, `ForgePath`, `distill_test.go`, `.fetchExaSearch`, `multiinstall_test.go`?**
...
- **Are the 111 inferred relationships involving `Open()` (e.g. with `collectStatusReport()` and `noteDistillSkip()`) actually correct?**
  _`Open()` has 111 INFERRED edges - model-reasoned connections that need verification._
...
- **Should `captureStdout` be split into smaller, more focused modules?**
  _Cohesion score 0.06863417982155114 - nodes in this community are weakly interconnected._