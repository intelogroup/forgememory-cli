...
var AgentsStorage = class extends chunk7Q5UQ5ES_cjs.VersionedStorageDomain {
  listKey = "agents";
...
var InMemoryAgentsStorage = class extends AgentsStorage {
  ...
  async list(args) {
    ...
    if (metadata && Object.keys(metadata).length > 0) {
      agents = agents.filter((agent) => {
        ...
        return Object.entries(metadata).every(([key, value]) => chunk3KZ2OT53_cjs.deepEqual(agent.metadata[key], value));
      });
    }
  ...
  async listVersions(input) {
    ...
    const perPage = chunk7Q5UQ5ES_cjs.normalizePerPage(perPageInput, 20);
    if (page < 0) {
      throw new Error("page must be >= 0");
    }
    const maxOffset = Number.MAX_SAFE_INTEGER / 2;
...
var FilesystemAgentsStorage = class extends AgentsStorage {
  ...
  constructor({ db }) {
    ...
    this.helpers = new chunk7Q5UQ5ES_cjs.FilesystemVersionedHelpers({
      db,
...
function agentIdFromSourcePath(path) {
  const prefix = `${chunk7Q5UQ5ES_cjs.SOURCE_CONTROL_AGENTS_DIR}/`;
...
var SourceAgentsSourceControl = class extends AgentsStorage {
  ...
  async discoverProviderAgentIds() {
    ...
    const files = await this.provider.listFiles({ path: chunk7Q5UQ5ES_cjs.SOURCE_CONTROL_AGENTS_DIR });
  ...
  async hydrateAgent(agentId) {
    ...
    const file = await this.provider.readFile({ path: chunk7Q5UQ5ES_cjs.getSourceAgentFilePath(agentId), ref });
    if (!file) {
  ...
  async persistSnapshot(agentId, snapshot, message) {
    ...
    return this.provider.writeFile({
      path: chunk7Q5UQ5ES_cjs.getSourceAgentFilePath(agentId),
...
`,
  ...
  async loadHistory(agentId) {
    ...
    const entries = await this.provider.listFileHistory({ path: chunk7Q5UQ5ES_cjs.getSourceAgentFilePath(agentId), ref: activeRef });
    const orderedEntries = [...entries].reverse();
    const versions = /* @__PURE__ */ new Map();
    let versionNumber = 0;
    for (const entry of orderedEntries) {
      const file = await this.provider.readFile({ path: chunk7Q5UQ5ES_cjs.getSourceAgentFilePath(agentId), ref: entry.ref ?? entry.id });
      if (!file) continue;
      const snapshot = parseJsonObject(file.content);
...
//# sourceMappingURL=chunk-TGB7JISJ.cjs.map
//# sourceMappingURL=chunk-TGB7JISJ.cjs.map