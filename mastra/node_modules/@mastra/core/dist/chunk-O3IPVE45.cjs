...
var chunk62FISUTS_cjs = require('./chunk-62FISUTS.cjs');
var chunkD324RFFU_cjs = require('./chunk-D324RFFU.cjs');
var chunk6E6FPM76_cjs = require('./chunk-6E6FPM76.cjs');
var chunkLP4WZA6D_cjs = require('./chunk-LP4WZA6D.cjs');
var chunkWSD4JNMB_cjs = require('./chunk-WSD4JNMB.cjs');
var chunk2TATDSHU_cjs = require('./chunk-2TATDSHU.cjs');
...
var ObservabilityStorage = class _ObservabilityStorage extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "OBSERVABILITY"
  ...
  async createSpan(_args) {
    throw new chunkXSOONORA_cjs.MastraError({
      ...
      domain: chunkXSOONORA_cjs.ErrorDomain.MASTRA_OBSERVABILITY,
  ...
  async getStructure(args) {
    if (this.getTraceLight === _ObservabilityStorage.prototype.getTraceLight) {
      throw new chunkXSOONORA_cjs.MastraError({
  ...
  async getTraceLight(args) {
    if (this.getStructure === _ObservabilityStorage.prototype.getStructure) {
      throw new chunkXSOONORA_cjs.MastraError({
  ...
  async getBranch(args) {
    const parsed = chunkQYKNPGZG_cjs.getBranchArgsSchema.parse(args);
    try {
      const skeleton = await this.getStructure({ traceId: parsed.traceId });
      if (!skeleton) return null;
      const branchSpanIds = chunkQYKNPGZG_cjs.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
      if (branchSpanIds.length === 0) return null;
      const { spans: spans2 } = await this.getSpans({ traceId: parsed.traceId, spanIds: branchSpanIds });
      if (spans2.length === 0) return null;
      spans2.sort((a, b) => a.startedAt.getTime() - b.startedAt.getTime());
      return { traceId: parsed.traceId, spans: spans2 };
    } catch (error) {
      const isFallbackTrigger = error instanceof chunkXSOONORA_cjs.MastraError && (error.id === "OBSERVABILITY_STORAGE_GET_STRUCTURE_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_TRACE_LIGHT_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_SPANS_NOT_IMPLEMENTED");
      if (!isFallbackTrigger) throw error;
    }
    const trace = await this.getTrace({ traceId: parsed.traceId });
    if (!trace) return null;
    const spans = chunkQYKNPGZG_cjs.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
...
function transformRow(row, tableName, options = {}) {
  ...
  const tableSchema = chunkQYKNPGZG_cjs.TABLE_SCHEMAS[tableName];
  const result = {};
...
function transformScoreRow(row, options = {}) {
  return transformRow(row, chunkQYKNPGZG_cjs.TABLE_SCORERS, options);
}
...
var ObservabilityInMemory = class extends ObservabilityStorage {
  ...
  deltaPollingFeatureEnabled() {
    return chunk6E6FPM76_cjs.coreFeatures.has(OBSERVABILITY_DELTA_POLLING_FEATURE);
  }
  ...
  upsertByIdField(records, cursorIds, record, idField) {
    ...
    if (id == null) {
      throw new chunkXSOONORA_cjs.MastraError({
        ...
        domain: chunkXSOONORA_cjs.ErrorDomain.STORAGE,
  ...
  maxMatchingCursorId(rows, cursorIds, matches) {
    ...
    for (const row of rows) {
      ...
      if (cursorId === void 0 || !matches(row)) {
        continue;
      }
  ...
  maybeRegisterBranchCursor(span) {
    if (!chunkQYKNPGZG_cjs.BRANCH_SPAN_TYPE_SET.has(span.spanType)) {
      return;
    }
  ...
  listAppendOnlyDelta(rows, cursorIds, matches, after, limit) {
    ...
    const matchingRows = rows.flatMap((row) => {
      const cursorId = cursorIds.get(row);
      if (cursorId === void 0 || cursorId <= afterCursorId || !matches(row)) {
        return [];
      }
      return [{ cursorId, row }];
  ...
  getTraceCursorId(traceId, filters) {
    ...
    if (cursorId === void 0 || !traceEntry?.rootSpan || !this.traceMatchesFilters(traceEntry, filters)) {
      return null;
    }
    return cursorId;
  }
  ...
  getMatchingRootSpans(args) {
    const { filters, pagination, orderBy } = chunkQYKNPGZG_cjs.listTracesArgsSchema.parse(args);
  ...
  async listTraces(args) {
    const { mode, filters, after, limit } = chunkQYKNPGZG_cjs.listTracesArgsSchema.parse(args);
    if (mode === "delta") {
      ...
      return {
        spans: chunkQYKNPGZG_cjs.toTraceSpans(deltaResponse.rows),
    ...
    return {
      spans: chunkQYKNPGZG_cjs.toTraceSpans(paged),
  ...
  async listBranches(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkQYKNPGZG_cjs.listBranchesArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      ...
      const afterCursorId = this.decodeDeltaCursor(after);
      const matches2 = Array.from(this.db.branchCursorIds.entries()).flatMap(([key, cursorId]) => {
        if (cursorId <= afterCursorId) {
          return [];
        }
        const [traceId, spanId] = key.split("\0");
      ...
      const deltaResponse = this.buildDeltaResponse(matches2, limit, fallbackCursorId);
      return {
        branches: deltaResponse.rows.map(chunkQYKNPGZG_cjs.toTraceSpan),
        delta: deltaResponse.delta,
        deltaCursor: deltaResponse.deltaCursor
      };
    }
    const allowedSpanTypes = filters?.spanType ? chunkQYKNPGZG_cjs.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : chunkQYKNPGZG_cjs.BRANCH_SPAN_TYPE_SET;
    const matches = [];
    for (const [, traceEntry] of this.db.traces) {
    ...
    matches.sort((a, b) => {
      if (sortField === "endedAt") {
        ...
        if (aVal == null) return sortDirection === "DESC" ? -1 : 1;
        if (bVal == null) return sortDirection === "DESC" ? 1 : -1;
        const diff2 = aVal.getTime() - bVal.getTime();
        return sortDirection === "DESC" ? -diff2 : diff2;
      }
      const diff = a.startedAt.getTime() - b.startedAt.getTime();
      return sortDirection === "DESC" ? -diff : diff;
    });
    ...
    return {
      ...
      branches: paged.map(chunkQYKNPGZG_cjs.toTraceSpan),
  ...
  spanMatchesBranchFilters(span, filters) {
    ...
    if (filters.parentEntityVersionId !== void 0 && span.parentEntityVersionId !== filters.parentEntityVersionId)
      return false;
    ...
    if (filters.rootEntityVersionId !== void 0 && span.rootEntityVersionId !== filters.rootEntityVersionId)
      return false;
    ...
    if (filters.status !== void 0) {
      const spanStatus = chunkQYKNPGZG_cjs.toTraceSpan(span).status;
  ...
  async listMetrics(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkLP4WZA6D_cjs.listMetricsArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
        this.db.metricRecords,
        this.db.metricCursorIds,
        (metric) => this.metricMatchesFilters(metric, filters),
        after,
        limit
      );
      return {
  ...
  async getMetricTimeSeries(args) {
    ...
    if (args.groupBy && args.groupBy.length > 0) {
      ...
      for (const m of filtered) {
        ...
        if (!entry) {
          entry = { displayName, buckets: /* @__PURE__ */ new Map() };
  ...
  *iterateObservabilityContextRecords() {
  ...
  async getEntityTypes(_args) {
    const validTypes = new Set(Object.values(chunkLP4WZA6D_cjs.EntityType));
    const typeSet = /* @__PURE__ */ new Set();
    for (const record of this.iterateObservabilityContextRecords()) {
      if (record.entityType && validTypes.has(record.entityType)) {
        typeSet.add(record.entityType);
      }
    }
  ...
  async listLogs(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkLP4WZA6D_cjs.listLogsArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
        this.db.logRecords,
  ...
  async listScores(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkLP4WZA6D_cjs.listScoresArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
  ...
  async listFeedback(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkLP4WZA6D_cjs.listFeedbackArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
...
var entityTypeValues = new Set(Object.values(chunkLP4WZA6D_cjs.EntityType));
...
// src/storage/domains/background-tasks/base.ts
var BackgroundTasksStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
...
// src/storage/domains/background-tasks/inmemory.ts
...
var BlobStore = class extends chunkWSD4JNMB_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
      name: "BLOBS"
...
// src/storage/domains/channels/base.ts
var ChannelsStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "CHANNELS"
...
// src/storage/domains/channels/inmemory.ts
var InMemoryChannelsStorage = class extends ChannelsStorage {
  ...
  async getInstallationByAgent(platform, agentId) {
    const statusPriority = { active: 0, pending: 1, error: 2 };
    let best = null;
...
function resolveZodSchema(zodString) {
  return Function("z", `"use strict";return (${zodString});`)(v4.z);
}
...
var DatasetsStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "DATASETS"
...
var DatasetsInMemory = class extends DatasetsStorage {
  ...
  async listDatasets(args) {
    ...
    const perPage = chunk7Q5UQ5ES_cjs.normalizePerPage(perPageInput, 100);
    const { offset: start, perPage: perPageForResponse } = chunk7Q5UQ5ES_cjs.calculatePagination(page, perPageInput, perPage);
...
var ExperimentsStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "EXPERIMENTS"
...
// src/storage/domains/harness/base.ts
var HarnessStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "HARNESS"
...
// src/storage/domains/harness/inmemory.ts
...
var MemoryStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  /**
...
var InMemoryMemory = class extends MemoryStorage {
  ...
  async listMessages({
    ...
    const perPage = chunk7Q5UQ5ES_cjs.normalizePerPage(perPageInput, 40);
    if (page < 0) {
      throw new Error("page must be >= 0");
    }
    ...
    const { offset, perPage: perPageForResponse } = chunk7Q5UQ5ES_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async listMessagesByResourceId({
    ...
    const list = new chunk2TATDSHU_cjs.MessageList().add(
  ...
  async saveMessages(args) {
    ...
    const list = new chunk2TATDSHU_cjs.MessageList().add(messages, "memory");
    return { messages: list.get.all.db() };
  }
...
var AGENT_THREAD_KEY_SEPARATOR = "\0";
var AGENT_THREAD_STREAM_TOPIC_PREFIX = "agent.thread-stream";
var defaultAgentThreadPubSub = new chunkD324RFFU_cjs.EventEmitterPubSub();
...
function createRuntimeState() {
  return {
    ...
    watchedThreadRunIds: /* @__PURE__ */ new Set(),
...
var AgentThreadStreamRuntime = class {
  ...
  #withBroadcastStream(output, pubsub, key) {
    const runtime = this;
    const parts = [];
  ...
  #getThreadTarget(options) {
    ...
    const threadId = options?.requestContext?.get(chunkPJIAL3WK_cjs.MASTRA_THREAD_ID_KEY) || (typeof thread === "string" ? thread : thread?.id);
    const resourceId = options?.requestContext?.get(chunkPJIAL3WK_cjs.MASTRA_RESOURCE_ID_KEY) || options?.memory?.resource;
  ...
  resetForTests() {
    ...
    defaultAgentThreadPubSub = new chunkD324RFFU_cjs.EventEmitterPubSub();
  }
  #resetState(pubsub) {
  ...
  async #drainPendingContinuations(state, pubsub, key) {
    if (state.activeThreadRunIds.has(key)) {
      return false;
    }
  ...
  #startContinuation(state, pubsub, key, pending) {
    ...
    void pending.agent.stream(pending.messages, {
  ...
  continueWithMessages(agent, messages, target, pubsub) {
    ...
    const runId = target.runId ?? crypto$1.randomUUID();
    const pending = {
      agent,
      messages,
      runId,
  ...
  async #drainPendingIdleSignals(state, pubsub, key) {
    if (state.activeThreadRunIds.has(key)) {
      return;
    }
  ...
  async #waitForRemoteRunToFinish(pubsub, key, runId) {
    ...
    await new Promise((resolve3) => {
      ...
      void resolvedPubSub.subscribe(topic, onEvent).catch(() => resolve3());
    });
  }
  async subscribeToThread(agent, options, pubsub) {
      ...
      return {
        ...
        output: {
          ...
          _waitUntilFinished: async () => {
            ...
            await new Promise((resolve3) => remoteRun.finishWaiters.push(resolve3));
          }
        },
        runId,
    ...
    const onEvent = (event) => {
      ...
      if (data.type === "signal-enqueued") {
        ...
        const queue = signalsByThread.get(key) ?? [];
        queue.push(chunk2TATDSHU_cjs.createSignal(data.signal));
      ...
      if (data.type === "run-completed" || data.type === "run-aborted" || data.type === "run-suspended") {
        if ((data.type !== "run-suspended" || !state.approvalSuspendedRunIds.has(data.runId)) && state.activeThreadRunIds.get(key) === data.runId) {
          state.activeThreadRunIds.delete(key);
        }
          ...
          while (!done || pendingRuns.length > 0) {
            ...
            try {
              while (true) {
                ...
                yield typedPart;
                if (done) break;
                if (typedPart.type === "finish" || typedPart.type === "error" || typedPart.type === "abort" || typedPart.type === "tool-call-suspended") {
                  readerReleased = true;
                  void (async () => {
                    try {
                      while (true) {
              ...
              if (!readerReleased && !done && cancelledByAbort) {
                yield { type: "abort", runId: run.runId };
  ...
  sendMessage(agent, message, target, pubsub) {
    return this.sendSignal(agent, chunk2TATDSHU_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() }), target, pubsub);
  }
  queueMessage(agent, message, target, pubsub) {
    const state = this.#getState(pubsub);
    const signal = chunk2TATDSHU_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    ...
    const queuedRunId = crypto$1.randomUUID();
  ...
  async sendStateSignal(agent, stateInput, target, pubsub) {
    ...
    const memoryContext = chunk62FISUTS_cjs.parseMemoryRequestContext(requestContext);
    const memory = await agent.getMemory({ requestContext });
    if (!memory) {
    ...
    const applied = await chunk62FISUTS_cjs.applyStateSignal({
      input: stateInput,
      memory,
      thread,
      resourceId,
  ...
  sendSignal(agent, signalInput, target, pubsub) {
    const state = this.#getState(pubsub);
    let signal = chunk2TATDSHU_cjs.createSignal({ ...signalInput, acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    let runId = target.runId;
    ...
    const resourceId = target.resourceId ?? activeRecord?.resourceId;
    const threadId = target.threadId ?? activeRecord?.threadId;
    signal = chunk2TATDSHU_cjs.resolveDeliveryAttributes(
      signal,
...
var agentThreadStreamRuntime = new AgentThreadStreamRuntime();
...
function notificationSummaryContents(summary) {
  const sources = Object.entries(summary.bySource).sort(([a], [b]) => a.localeCompare(b)).map(([source, count]) => `${source}: ${count}`).join(", ");
...
function notificationSummarySignalMetadata(summary) {
  ...
  return {
    ...
    groups: Object.entries(summary.bySource).sort(([a], [b]) => a.localeCompare(b)).map(([source, count]) => ({ source, count })),
...
function createNotificationSignal(notification) {
  return chunk2TATDSHU_cjs.createSignal({
    type: "notification",
...
function summarizeNotifications(notifications) {
  ...
  return pendingNotifications.reduce(
...
var isSummaryDue = (record, now) => Boolean(record.summaryAt && record.summaryAt.getTime() <= now.getTime());
...
var compareDueDispatchItems = (a, b) => deliveryPriority[a.priority] - deliveryPriority[b.priority] || a.createdAt.getTime() - b.createdAt.getTime();
var getHighestPriority = (records) => records.reduce(
  (highest, record) => deliveryPriority[record.priority] < deliveryPriority[highest] ? record.priority : highest,
  "low"
);
var getEarliestCreatedAt = (records) => records.reduce(
...
async function sendNotificationRecord({
  ...
  const signal = createNotificationSignal({
    ...
    status: "delivered",
    deliveredAt: now});
  const target = { resourceId: current.resourceId, threadId: current.threadId };
...
async function sendNotificationSummary({
  ...
  const target = records.every((record) => record.priority === "low") ? { resourceId: first.resourceId, threadId: first.threadId, ifIdle: { behavior: "persist" } } : { resourceId: first.resourceId, threadId: first.threadId };
  const result = agent.sendSignal(signal, target);
  await result.persisted;
  if (!result.accepted) {
...
async function dispatchDueNotifications({
  ...
  ungroupedIndividual.sort(
    (a, b) => compareDueDispatchItems(
...
var NotificationsStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({ component: "STORAGE", name: "NOTIFICATIONS" });
  }
};
var cloneDate = (value) => value ? new Date(value) : void 0;
var notificationKey = (threadId, id) => `${threadId}\0${id}`;
...
var InMemoryNotificationsStorage = class extends NotificationsStorage {
  ...
  async createNotification(input) {
    ...
    if (existing) {
      ...
      const next = {
        ...
        attributes: input.attributes ? { ...cloneValue(existing.attributes), ...cloneValue(input.attributes) } : cloneValue(existing.attributes),
        updatedAt: now2,
        ...
        metadata: input.metadata ? { ...cloneValue(existing.metadata), ...cloneValue(input.metadata) } : cloneValue(existing.metadata)
      };
  ...
  findCoalescable(input) {
    ...
    return [...this.#notifications.values()].find((record) => {
      if (record.threadId !== input.threadId || record.source !== input.source || record.kind !== input.kind || record.status !== "pending")
        return false;
      if (record.agentId !== input.agentId || record.resourceId !== input.resourceId) return false;
      return Boolean(
        input.dedupeKey && record.dedupeKey === input.dedupeKey || input.coalesceKey && record.coalesceKey === input.coalesceKey
      );
    });
  }
};

// src/storage/domains/schedules/base.ts
var SchedulesStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCHEDULES"
...
// src/storage/domains/schedules/inmemory.ts
...
var InMemorySchedulesStorage = class extends SchedulesStorage {
  ...
  async recordTrigger(trigger) {
    const stored = {
      ...trigger,
      id: trigger.id ?? crypto$1.randomUUID(),
...
var ScoresStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCORES"
...
var ScoresInMemory = class extends ScoresStorage {
  ...
  async listScoresByScorerId({
    ...
    const perPage = chunk7Q5UQ5ES_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
    const { offset: start, perPage: perPageForResponse } = chunk7Q5UQ5ES_cjs.calculatePagination(page, perPageInput, perPage);
...
// src/storage/domains/tool-provider-connections/base.ts
var ToolProviderConnectionsStorage = class extends chunkWSD4JNMB_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
...
// src/storage/domains/tool-provider-connections/inmemory.ts
function connKey(authorId, providerId, connectionId) {
  return `${authorId}\0${providerId}\0${connectionId}`;
}
...
// src/storage/workflow-snapshot.ts
...
var WorkflowsStorage = class extends chunk7Q5UQ5ES_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "WORKFLOWS"
...
function deepCloneForRun(value, seen) {
  ...
  if (value instanceof Map) {
    ...
    for (const [k, v] of value) {
      out2.set(deepCloneForRun(k, seen), deepCloneForRun(v, seen));
    }
    return out2;
  }
  if (value instanceof Set) {
    ...
    for (const v of value) {
      out2.add(deepCloneForRun(v, seen));
    }
    return out2;
  }
  ...
  if (value instanceof Error) {
    ...
    Object.defineProperty(out2, "message", {
      value: value.message,
      writable: true,
      configurable: true,
      enumerable: true
    });
    Object.defineProperty(out2, "name", { value: value.name, writable: true, configurable: true });
    ...
    if (includeStack) {
      Object.defineProperty(out2, "stack", { value: value.stack, writable: true, configurable: true });
    }
    seen.set(value, out2);
    const outRecord = out2;
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      out2[i] = deepCloneForRun(value[i], seen);
    }
    return out2;
  }
  ...
  seen.set(value, out);
  for (const key of Object.keys(value)) {
    out[key] = deepCloneForRun(value[key], seen);
  }
  return out;
}
var WorkflowsInMemory = class extends WorkflowsStorage {
  ...
  async listWorkflowRuns({
    ...
    if (perPage !== void 0 && page !== void 0) {
      const normalizedPerPage = chunk7Q5UQ5ES_cjs.normalizePerPage(perPage, Number.MAX_SAFE_INTEGER);
...
// src/storage/mock.ts
var InMemoryStore = class extends chunk7Q5UQ5ES_cjs.MastraCompositeStore {
  stores;
  /**
  ...
  constructor({ id = "in-memory" } = {}) {
    ...
    this.#db = new chunkTGB7JISJ_cjs.InMemoryDB();
    this.stores = {
      ...
      agents: new chunkTGB7JISJ_cjs.InMemoryAgentsStorage({ db: this.#db }),
      ...
      promptBlocks: new chunk4MGP65EJ_cjs.InMemoryPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkUMUFUXPE_cjs.InMemoryScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkEYSZBWIV_cjs.InMemoryMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkGVRZI3NY_cjs.InMemoryMCPServersStorage({ db: this.#db }),
      workspaces: new chunkKMXG2T6C_cjs.InMemoryWorkspacesStorage({ db: this.#db }),
      skills: new chunkG4L2S65S_cjs.InMemorySkillsStorage({ db: this.#db }),
      favorites: new chunkGJE2FI3V_cjs.InMemoryFavoritesStorage({ db: this.#db }),
      ...
      threadState: new chunk7Q5UQ5ES_cjs.InMemoryThreadStateStorage()
    };
  }
  /**
...
var FilesystemDB = class {
  ...
  listDomainFiles(directory, extension = ".json") {
    const baseDir = path.resolve(this.dir, directory);
    const rootDir = path.resolve(this.dir);
    if (!baseDir.startsWith(rootDir + path.sep) && baseDir !== rootDir) {
    ...
    return fs.readdirSync(baseDir).filter((file) => path.extname(file) === extension && fs.statSync(path.join(baseDir, file)).isFile()).map((file) => `${directory}/${file}`);
  }
  /**
...
var FilesystemStore = class extends chunk7Q5UQ5ES_cjs.MastraCompositeStore {
  #db;
  #dir;
  constructor(config = {}) {
    ...
    this.stores = {
      agents: new chunkTGB7JISJ_cjs.FilesystemAgentsStorage({ db: this.#db }),
      promptBlocks: new chunk4MGP65EJ_cjs.FilesystemPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkUMUFUXPE_cjs.FilesystemScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkEYSZBWIV_cjs.FilesystemMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkGVRZI3NY_cjs.FilesystemMCPServersStorage({ db: this.#db }),
      workspaces: new chunkKMXG2T6C_cjs.FilesystemWorkspacesStorage({ db: this.#db }),
      skills: new chunkG4L2S65S_cjs.FilesystemSkillsStorage({ db: this.#db })
    };
  }
  /**
...
// src/storage/providers/github.ts
var GitHubSourceControlProvider = class {
  ...
  async listFiles(input) {
    const query = new URLSearchParams({ path: this.sourcePath(input.path) });
    if (input.ref) query.set("ref", input.ref);
    const files = await this.request(`/files/list?${query.toString()}`);
...
var StoreOperations = class extends chunkWSD4JNMB_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
      name: "OPERATIONS"
...
var StoreOperationsInMemory = class extends StoreOperations {
  ...
  async insert({ tableName, record }) {
    ...
    let key = record.id;
    if (tableName === chunkQYKNPGZG_cjs.TABLE_NOTIFICATIONS && record.threadId && record.id) {
      key = `${record.threadId}\0${record.id}`;
    } else if ([chunkQYKNPGZG_cjs.TABLE_WORKFLOW_SNAPSHOT].includes(tableName) && !record.id && record.run_id) {
...
exports.BackgroundTasksInMemory = BackgroundTasksInMemory;
exports.BackgroundTasksStorage = BackgroundTasksStorage;
exports.BlobStore = BlobStore;
exports.ChannelsStorage = ChannelsStorage;
...
exports.GitHubSourceControlProvider = GitHubSourceControlProvider;
exports.HarnessStorage = HarnessStorage;
exports.InMemoryBlobStore = InMemoryBlobStore;
exports.InMemoryChannelsStorage = InMemoryChannelsStorage;
exports.InMemoryHarness = InMemoryHarness;
exports.InMemoryMemory = InMemoryMemory;
exports.InMemoryNotificationsStorage = InMemoryNotificationsStorage;
exports.InMemorySchedulesStorage = InMemorySchedulesStorage;
exports.InMemoryStore = InMemoryStore;
exports.InMemoryToolProviderConnectionsStorage = InMemoryToolProviderConnectionsStorage;
exports.MemoryStorage = MemoryStorage;
exports.MockStore = MockStore;
exports.NotificationsStorage = NotificationsStorage;
exports.ObservabilityInMemory = ObservabilityInMemory;
exports.ObservabilityStorage = ObservabilityStorage;
exports.SchedulesStorage = SchedulesStorage;
...
exports.ToolProviderConnectionsStorage = ToolProviderConnectionsStorage;
exports.WorkflowsInMemory = WorkflowsInMemory;
exports.WorkflowsStorage = WorkflowsStorage;
exports.agentThreadStreamRuntime = agentThreadStreamRuntime;
exports.buildCreateSpanRecord = buildCreateSpanRecord;
exports.buildFeedbackRecord = buildFeedbackRecord;
exports.buildLogRecord = buildLogRecord;
exports.buildMetricRecord = buildMetricRecord;
exports.buildScoreRecord = buildScoreRecord;
exports.buildUpdateSpanRecord = buildUpdateSpanRecord;
exports.cloneRunData = cloneRunData;
exports.createEmptyWorkflowSnapshot = createEmptyWorkflowSnapshot;
exports.createGitHubSourceControlProviderFromEnv = createGitHubSourceControlProviderFromEnv;
exports.createNotificationSignal = createNotificationSignal;
exports.createNotificationSummarySignal = createNotificationSummarySignal;
...
exports.dispatchDueNotifications = dispatchDueNotifications;
...
exports.notificationSignalAttributes = notificationSignalAttributes;
exports.notificationSignalMetadata = notificationSignalMetadata;
exports.notificationSummaryContents = notificationSummaryContents;
exports.notificationSummarySignalMetadata = notificationSummarySignalMetadata;
exports.safelyParseJSON = safelyParseJSON;
exports.serializeDate = serializeDate;
exports.serializeSpanAttributes = serializeSpanAttributes;
exports.summarizeNotifications = summarizeNotifications;
exports.toEntityType = toEntityType;
exports.transformRow = transformRow;
exports.transformScoreRow = transformScoreRow;
//# sourceMappingURL=chunk-O3IPVE45.cjs.map
//# sourceMappingURL=chunk-O3IPVE45.cjs.map