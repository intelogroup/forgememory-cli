...
- [Signal providers](references/docs-agents-signal-providers.md) - Learn what signal providers are and how to build one that monitors an external source and pushes notifications into agent threads.
...
- [Browser recording (alpha)](references/docs-browser-recording.md) - Record browser sessions as Motion-JPEG AVI videos with optional agent captions.
...
- [Background task streaming](references/docs-streaming-background-task-streaming.md) - Learn how to subscribe to background task lifecycle events with `BackgroundTaskManager.stream()` to monitor task status across agents, threads, and runs.
...
- [Reference: ProviderHistoryCompat](references/reference-processors-provider-history-compat.md) - API reference for the ProviderHistoryCompat processor in Mastra, which applies provider-specific history compatibility fixes.
- [Reference: RegexFilterProcessor](references/reference-processors-regex-filter-processor.md) - API reference for RegexFilterProcessor, a zero-cost regex-based content filter for blocking, redacting, or warning on pattern matches in agent messages.
- [Reference: ResponseCache](references/reference-processors-response-cache.md) - API reference for the ResponseCache input processor that caches LLM responses inside the agentic loop.
...
- [Reference: StreamErrorRetryProcessor](references/reference-processors-stream-error-retry-processor.md) - API reference for the StreamErrorRetryProcessor processor in Mastra, which retries transient stream errors.
...
- [Reference: PubSub](references/reference-pubsub-base.md) - API reference for the PubSub abstract base class that defines the pub/sub contract in Mastra.
- [Reference: CachingPubSub](references/reference-pubsub-caching-pubsub.md) - API reference for CachingPubSub, a pub/sub wrapper that adds event caching and replay for resumable streams.
- [Reference: EventEmitterPubSub](references/reference-pubsub-event-emitter.md) - API reference for EventEmitterPubSub, the default in-process pub/sub implementation in Mastra.
- [Reference: GoogleCloudPubSub](references/reference-pubsub-google-cloud-pubsub.md) - API reference for GoogleCloudPubSub, the Google Cloud Pub/Sub implementation for distributed event delivery.
- [Reference: RedisStreamsPubSub](references/reference-pubsub-redis-streams.md) - API reference for RedisStreamsPubSub, the Redis Streams pub/sub implementation for distributed event delivery.
- [Reference: UnixSocketPubSub](references/reference-pubsub-unix-socket-pubsub.md) - API reference for UnixSocketPubSub, the Unix socket pub/sub implementation for cross-process delivery on one host.
...
- [Reference: createNotificationInboxTool()](references/reference-signals-create-notification-inbox-tool.md) - API reference for createNotificationInboxTool(), which creates a tool for reading and managing notification inbox records.
- [Reference: SignalProvider](references/reference-signals-signal-provider.md) - API reference for SignalProvider, the abstract base class for building signal providers that monitor external sources and push events into agent threads.
- [Reference: TaskSignalProvider](references/reference-signals-task-signal-provider.md) - API reference for TaskSignalProvider, a signal provider that bundles the built-in task tools and the task state-signal processor behind a single agent registration.
- [Reference: WebhookSignalProvider](references/reference-signals-webhook-signal-provider.md) - API reference for WebhookSignalProvider, a push-based signal provider that routes incoming webhook payloads to subscribed agent threads.
...
- [Reference: Agent.streamUntilIdle()](references/reference-streaming-agents-streamUntilIdle.md) - API reference for Agent.streamUntilIdle(), which keeps an agent stream open until all dispatched background tasks have completed.
...
- [Reference: createCodeMode()](references/reference-tools-create-code-mode.md) - API reference for createCodeMode(), used to run multi-tool computations in a sandbox.
...
- [Reference: Workflow state reader](references/reference-workflows-workflow-state-reader.md) - API reference for workflow state reader helpers, which inspect persisted workflow run state.
