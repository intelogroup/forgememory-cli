...
function getStateSignalsMetadata(threadMetadata) {
  ...
  const mastra = threadMetadata.mastra;
  if (!isPlainObject2(mastra)) return {};
  const stateSignals = mastra.stateSignals;
  return isPlainObject2(stateSignals) ? stateSignals : {};
}
function setStateSignalMetadata(threadMetadata, stateId, tracking) {
  const existing = threadMetadata ?? {};
  const existingMastra = isPlainObject2(existing.mastra) ? existing.mastra : {};
  const existingStateSignals = isPlainObject2(existingMastra.stateSignals) ? existingMastra.stateSignals : {};
  return {
    ...existing,
    mastra: {
      ...existingMastra,
...
function sortStateSignals(signals) {
  return signals.map((signal, index) => ({ signal, index })).sort((left, right) => signalCreatedAt(left.signal) - signalCreatedAt(right.signal) || left.index - right.index).map(({ signal }) => signal);
}
function dbMessagesToStateSignals(messages, stateId, threadId) {
  return sortStateSignals(
    messages.filter((message) => message.role === "signal").map((message) => {
      try {
        return chunk2TATDSHU_cjs.mastraDBMessageToSignal(message);
      } catch {
        return void 0;
      }
    }).filter(
      (signal) => signal?.type === "state" && isPlainObject2(signal.metadata?.state) && (!stateId || signal.metadata.state.id === stateId) && signal.metadata.state.threadId === threadId
    )
  );
}
...
async function resolveStateSignalHistory({
  messageList,
  memory,
  threadId,
  resourceId,
...
function createStateSignalInput(input, options) {
  ...
  const signal = chunk2TATDSHU_cjs.createSignal({
    ...
    metadata: {
      ...metadata,
      state: {
        ...isPlainObject2(metadata?.state) ? metadata.state : {},
        id: stateId,
        cacheKey,
        mode
      },
...
async function applyStateSignal({
  input,
  memory,
  thread,
  resourceId,
  ...
  const hasActiveCopy = activeSignals.some(
    (signal2) => signal2.metadata?.state?.cacheKey === cacheKey && signal2.metadata?.state?.mode === mode
  );
  ...
  const updatedSignal = chunk2TATDSHU_cjs.createSignal({
    ...signal,
    metadata: {
      ...signal.metadata,
      state: {
        ...isPlainObject2(signal.metadata?.state) ? signal.metadata.state : {},
        id: stateId,
        threadId,
        cacheKey,
        version,
  ...
  const nextTracking = {
    ...
    activeCopies: updatedActiveSignals.map((activeSignal) => {
      const activeStateMetadata = isPlainObject2(activeSignal.metadata?.state) ? activeSignal.metadata.state : {};
      return {
        id: activeSignal.id,
...
exports.applyStateSignal = applyStateSignal;
exports.getStateSignalsMetadata = getStateSignalsMetadata;
...
exports.resolveStateSignalHistory = resolveStateSignalHistory;
exports.setThreadOMMetadata = setThreadOMMetadata;
//# sourceMappingURL=chunk-62FISUTS.cjs.map
//# sourceMappingURL=chunk-62FISUTS.cjs.map