...
var getTraceStep = chunk6ZBUYPOD_cjs.createStep2({
  id: "__process-trace-scoring",
  inputSchema: v4.z.object({
    targets: v4.z.array(
      v4.z.object({
        traceId: v4.z.string(),
        spanId: v4.z.string().optional()
      })
    ),
    scorerId: v4.z.string()
  }),
      ...
      const mastraError = new chunkXSOONORA_cjs.MastraError(
        {
          ...
          domain: chunkXSOONORA_cjs.ErrorDomain.SCORER,
          category: chunkXSOONORA_cjs.ErrorCategory.SYSTEM,
...
async function runScorerOnTarget({
  ...
  const result = await scorer.run({
    ...
    scoreSource: "trace",
    targetScope: "span",
    targetEntityType: chunkLP4WZA6D_cjs.getEntityTypeForSpan(span),
...
async function validateAndSaveScore({ storage, scorerResult }) {
  ...
  const payloadToSave = chunkQYKNPGZG_cjs.saveScorePayloadSchema.parse(scorerResult);
...
var scoreTracesWorkflow = chunk6ZBUYPOD_cjs.createWorkflow2({
