...
// src/storage/domains/thread-state/base.ts
var ThreadStateStorage = class extends chunkWSD4JNMB_cjs.MastraBase {
  constructor() {
    super({
      component: "STORAGE",
      name: "THREAD_STATE"
...
// src/storage/domains/thread-state/inmemory.ts
function clone(value) {
  return value === void 0 ? value : structuredClone(value);
}
var InMemoryThreadStateStorage = class extends ThreadStateStorage {
  stateByThread = /* @__PURE__ */ new Map();
  async init() {
  }
...
var MastraCompositeStore = class extends chunkWSD4JNMB_cjs.MastraBase {
  hasInitialized = null;
   ...
   * @internal
   */
  __registerMastra(mastra, seen = /* @__PURE__ */ new Set()) {
    if (seen.has(this)) return;
    seen.add(this);
    this.mastra = mastra;
    const cascade = (target) => {
      if (!target || typeof target !== "object" || seen.has(target)) return;
      const fn = target.__registerMastra;
...
var StorageDomain = class extends chunkWSD4JNMB_cjs.MastraBase {
  /**
...
// src/storage/source-control.ts
var SOURCE_CONTROL_AGENTS_DIR = "agents";
function getSourceControlEntityFilePath(directory, entityId) {
  return `${directory}/${encodeURIComponent(entityId)}.json`;
}
function getSourceAgentFilePath(agentId) {
...
function stableSortKeys(value) {
  ...
  if (value && typeof value === "object" && !(value instanceof Date)) {
    return Object.fromEntries(
      Object.entries(value).filter(([, entry]) => entry !== void 0).sort(([left], [right]) => left.localeCompare(right)).map(([key, entry]) => [key, stableSortKeys(entry)])
    );
  }
  return value;
}
var FilesystemVersionedHelpers = class _FilesystemVersionedHelpers {
  ...
  gitHistoryLimit;
  perEntityFilesDir;
  shouldPersistToPerEntityFile;
  perEntitySnapshotFilter;
  /**
  ...
  entityIdFromPerEntityFilename(filename) {
  ...
  hydrate() {
    ...
    this.hydrated = true;
    const hydrateSnapshot = (entityId, snapshotConfig) => {
  ...
  async loadGitHistory() {
    ...
    if (this.perEntityFilesDir) {
      const perEntityIds = /* @__PURE__ */ new Set();
      for (const filename of this.db.listDomainFiles(this.perEntityFilesDir)) {
   ...
   */
  async loadPerEntityGitHistory(entityId, startCount) {
    const git = _FilesystemVersionedHelpers.gitHistory;
    const dir = this.db.dir;
    ...
    let previousSnapshotForEntity;
    let count = startCount;
    for (const commit of orderedPerEntity) {
      const snapshotConfig = await git.getFileAtCommit(dir, commit.hash, filename);
      if (!snapshotConfig || typeof snapshotConfig !== "object") {
        previousSnapshotForEntity = void 0;
        continue;
      }
      const serialized = JSON.stringify(snapshotConfig);
  ...
  persistToDisk() {
    const diskData = {};
    const perEntityData = /* @__PURE__ */ new Map();
    for (const [entityId, entity] of this.entities) {
      ...
      if (this.perEntityFilesDir && this.shouldPersistToPerEntityFile?.(entity)) {
        const filtered = this.perEntitySnapshotFilter ? this.perEntitySnapshotFilter(snapshotConfig, entity) : snapshotConfig;
        perEntityData.set(entityId, stableSortKeys(filtered));
      } else {
        diskData[entityId] = stableSortKeys(snapshotConfig);
      }
    }
   ...
   */
  async ensurePerEntityGitHistory(entityId) {
...
exports.InMemoryThreadStateStorage = InMemoryThreadStateStorage;
exports.MastraCompositeStore = MastraCompositeStore;
exports.MastraStorage = MastraStorage;
exports.SOURCE_CONTROL_AGENTS_DIR = SOURCE_CONTROL_AGENTS_DIR;
exports.StorageDomain = StorageDomain;
exports.ThreadStateStorage = ThreadStateStorage;
exports.VersionedStorageDomain = VersionedStorageDomain;
exports.calculatePagination = calculatePagination;
exports.getSourceAgentFilePath = getSourceAgentFilePath;
exports.getSourceControlEntityFilePath = getSourceControlEntityFilePath;
exports.normalizePerPage = normalizePerPage;
//# sourceMappingURL=chunk-7Q5UQ5ES.cjs.map
//# sourceMappingURL=chunk-7Q5UQ5ES.cjs.map