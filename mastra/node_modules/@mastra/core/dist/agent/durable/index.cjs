...
var chunkGMFOMFCM_cjs = require('../../chunk-GMFOMFCM.cjs');
var chunk6ZBUYPOD_cjs = require('../../chunk-6ZBUYPOD.cjs');
var chunkZCBG4ZQT_cjs = require('../../chunk-ZCBG4ZQT.cjs');
var chunkUHTO5JDE_cjs = require('../../chunk-UHTO5JDE.cjs');
var chunkYLB2YBKQ_cjs = require('../../chunk-YLB2YBKQ.cjs');
var chunkD324RFFU_cjs = require('../../chunk-D324RFFU.cjs');
var chunk6QR4RQID_cjs = require('../../chunk-6QR4RQID.cjs');
var chunk3KZ2OT53_cjs = require('../../chunk-3KZ2OT53.cjs');
var chunkY4FZMBP5_cjs = require('../../chunk-Y4FZMBP5.cjs');
var chunkFHPG32XN_cjs = require('../../chunk-FHPG32XN.cjs');
var chunkLP4WZA6D_cjs = require('../../chunk-LP4WZA6D.cjs');
var chunkVRAH6SOV_cjs = require('../../chunk-VRAH6SOV.cjs');
...
async function resolveScope(agent, mergedOptions) {
  const requestContext = mergedOptions?.requestContext ?? new chunkPJIAL3WK_cjs.RequestContext();
  const memory = await agent.getMemory();
  if (!memory) return null;
  const threadIdFromContext = requestContext.get(chunkPJIAL3WK_cjs.MASTRA_THREAD_ID_KEY);
  const resourceIdFromContext = requestContext.get(chunkPJIAL3WK_cjs.MASTRA_RESOURCE_ID_KEY);
...
async function runDurableStreamUntilIdle(agent, messages, streamOptions, deps) {
  ...
  const mergedOptions = chunk3KZ2OT53_cjs.deepMerge(
    defaultOptions,
    restStreamOptions ?? {}
  );
...
async function prepareForDurableExecution(options) {
  ...
  const requestContext = providedRequestContext ?? new chunkPJIAL3WK_cjs.RequestContext();
  const requestVersions = requestContext.get(chunkPJIAL3WK_cjs.MASTRA_VERSIONS_KEY);
  let mergedVersions = chunkPJIAL3WK_cjs.mergeVersionOverrides(mastra?.getVersionOverrides?.(), requestVersions);
  if (execOptions?.versions) {
    mergedVersions = chunkPJIAL3WK_cjs.mergeVersionOverrides(mergedVersions, execOptions.versions);
  }
  if (mergedVersions) {
    requestContext.set(chunkPJIAL3WK_cjs.MASTRA_VERSIONS_KEY, mergedVersions);
  }
  ...
  const messageList = new chunk2TATDSHU_cjs.MessageList({
    threadId,
    resourceId
  });
  ...
  if (inputProcessors.length > 0) {
    try {
      ...
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-HNUPPQ27.cjs');
  ...
  const saveQueueManager = memory ? new chunk6ZBUYPOD_cjs.SaveQueueManager({
    logger,
    memory
  }) : void 0;
...
function createDurableAgentStream(options) {
    ...
    try {
      switch (streamEvent.type) {
        case AgentStreamEventTypes.CHUNK: {
          ...
          chunkUHTO5JDE_cjs.safeEnqueue(controller, chunk);
          await onChunk?.(chunk);
          break;
        }
        ...
        case AgentStreamEventTypes.FINISH: {
          ...
          chunkUHTO5JDE_cjs.safeEnqueue(controller, finishChunk);
          chunkUHTO5JDE_cjs.safeClose(controller);
          try {
            await onFinish?.(data);
          } catch (callbackError) {
  ...
  const output = new chunkUHTO5JDE_cjs.MastraModelOutput({
    model,
    stream,
    messageList,
    messageId,
...
function createDurableBackgroundTaskCheckStep() {
  return chunk6ZBUYPOD_cjs.createStep({
    id: BG_CHECK_STEP_ID,
...
async function resolveRuntimeDependencies(options) {
  ...
  if (globalEntry) {
    ...
    try {
      ...
      const resolveRequestContext = new chunkPJIAL3WK_cjs.RequestContext();
  ...
  if (memory) {
    saveQueueManager = new chunk6ZBUYPOD_cjs.SaveQueueManager({
...
async function toolRequiresApproval(tool, globalRequireApproval, args) {
  ...
  const needsApprovalFn = chunkY4FZMBP5_cjs.getNeedsApprovalFn(tool);
  if (needsApprovalFn) {
    try {
...
async function resolveModelFromConfig(config, mastra) {
  const requestContext = new chunkPJIAL3WK_cjs.RequestContext();
  const modelConfigString = config.originalConfig ?? `${config.provider}/${config.modelId}`;
  if (typeof modelConfigString === "string") {
    return await chunkYLB2YBKQ_cjs.resolveModelConfig(modelConfigString, requestContext, mastra);
  }
  return await chunkYLB2YBKQ_cjs.resolveModelConfig(
    modelConfigString,
    requestContext,
    mastra
  );
      ...
      for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (!chunkUHTO5JDE_cjs.isSupportedLanguageModel(model)) {
            ...
            let currentProviderOptions = chunk6ZBUYPOD_cjs.mergeProviderOptions(
              execOptions.providerOptions,
              modelEntry.config.providerOptions
            );
            const observability = mastra?.observability?.getSelectedInstance({ requestContext });
            const inputModelSpanData = inputData.modelSpanData;
            ...
            const structuredOutput = structuredOutputConfig?.schema && !structuredOutputConfig?.structuringModelConfig ? {
            ...
            if (registryEntry?.inputProcessors?.length) {
              ...
              const runner = new chunkUHTO5JDE_cjs.ProcessorRunner({
              ...
              currentModel = processInputStepResult.model ?? currentModel;
              currentTools = processInputStepResult.tools ?? currentTools;
              currentToolChoice = processInputStepResult.toolChoice;
            ...
            const modelResult = chunk6ZBUYPOD_cjs.execute({
              runId,
              model: currentModel,
              providerOptions: currentProviderOptions,
              inputMessages,
            ...
            const outputStream = new chunkUHTO5JDE_cjs.MastraModelOutput({
              model: {
            ...
            if (registryEntry?.errorProcessors?.length) {
              try {
                ...
                const currentMessageList = new chunk2TATDSHU_cjs.MessageList();
...
function createDurableToolCallStep() {
  return chunk6ZBUYPOD_cjs.createStep({
    ...
    execute: async (params) => {
      ...
      const toolKey = registryEntry?.tools?.[toolName] ? toolName : Object.entries(registryEntry?.tools ?? {}).find(([, registeredTool]) => registeredTool === tool)?.[0];
      ...
      const doFlush = () => flushMessagesBeforeSuspension({
        saveQueueManager,
        messageList,
        memory,
        threadId: state?.threadId,
      ...
      const isResumingFromSuspension = resumeData && typeof resumeData === "object" && resumeData !== null && !("approved" in resumeData);
      const bgManager = registryEntry?.backgroundTaskManager;
      const bgConfig = registryEntry?.backgroundTasksConfig;
      ...
      if (bgManager && !bgConfig?.disabled && typeof cleanedArgs === "object" && cleanedArgs !== null) {
        const bgResolved = chunkVRAH6SOV_cjs.resolveBackgroundConfig({
          llmBgOverrides,
          toolName,
        ...
        if (bgResolved.runInBackground) {
          try {
            const bgTask = chunkVRAH6SOV_cjs.createBackgroundTask(bgManager, {
              toolName,
              toolCallId,
              args: cleanedArgs,
...
function createDurableLLMMappingStep() {
  return chunk6ZBUYPOD_cjs.createStep({
    ...
    execute: async ({ inputData }) => {
      ...
      const allToolsErrored = toolResults.length > 0 && toolResults.every((r) => r.error !== void 0);
...
function createDurableAgenticWorkflow(options) {
  ...
  const singleIterationWorkflow = chunk6ZBUYPOD_cjs.createWorkflow({
  ...
  return chunk6ZBUYPOD_cjs.createWorkflow({
    ...
    async (params) => {
      ...
      if (registryEntry?.outputProcessors?.length) {
        try {
          ...
          const outputMessageList = new chunk2TATDSHU_cjs.MessageList();
          outputMessageList.deserialize(state.messageListState);
          await runner.runOutputProcessors(outputMessageList, {}, requestContext ?? new chunkPJIAL3WK_cjs.RequestContext(), 0);
        } catch (error) {
          logger?.warn?.(`[DurableAgent] Error running output processors: ${error}`);
        }
      }
      const durableState = initData.state;
      if (registryEntry?.saveQueueManager && registryEntry.memory && durableState?.threadId && durableState?.resourceId && !durableState.observationalMemory) {
        try {
          const memoryMessageList = new chunk2TATDSHU_cjs.MessageList();
    ...
    async (params) => {
      ...
      const logger = mastra?.getLogger?.();
      const inputMessageList = new chunk2TATDSHU_cjs.MessageList();
      ...
      const resolveContext = requestContext ?? new chunkPJIAL3WK_cjs.RequestContext();
      for (const [scorerKey, scorerEntry] of Object.entries(scorers)) {
        const { scorerName, sampling } = scorerEntry;
        try {
          const scorer = mastra?.getScorer?.(scorerName);
          if (!scorer) {
          ...
          chunk6ZBUYPOD_cjs.runScorer({
            ...
            threadId: initData.state?.threadId,
            resourceId: initData.state?.resourceId,
            ...chunkFHPG32XN_cjs.createObservabilityContext(tracingContext)
          });
        } catch (error) {
...
var DurableAgent = class extends chunk6ZBUYPOD_cjs.Agent {
  /** The wrapped agent */
  #wrappedAgent;
  ...
  constructor(config) {
    ...
    this.#innerPubsub = pubsub ?? new chunkD324RFFU_cjs.EventEmitterPubSub();
   ...
   */
    ...
    if (this.#cacheConfig === false) {
      ...
      this.#cachingPubsub = new chunkGMFOMFCM_cjs.CachingPubSub(this.#innerPubsub, resolvedCache);
    }
  }
  // ===========================================================================
  ...
  async stream(messages, options) {
    ...
    ready.then(() => this.executeWorkflow(runId, workflowInput)).catch((error) => {
  ...
  #clearPubsubTopic(runId) {
    ...
    if ("clearTopic" in pubsub && typeof pubsub.clearTopic === "function") {
      void pubsub.clearTopic(AGENT_STREAM_TOPIC(runId));
    }
  }
  /**
