...
var chunkGBRVBJXF_cjs = require('./chunk-GBRVBJXF.cjs');
...
var CoreToolBuilder = class extends chunkWSD4JNMB_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkY4FZMBP5_cjs.isVercelTool(this.originalTool) && !chunkY4FZMBP5_cjs.isProviderDefinedTool(this.originalTool)) {
        ...
        if (chunkCMKSC37Z_cjs.isZodObject(schema) && isZodV4Schema(schema)) {
          ...
          if (isBackgroundEligible) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
              _background: chunkVRAH6SOV_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
          ...
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            ...
            const injectedKeys = [];
            if (isBackgroundEligible) {
              properties._background = chunkVRAH6SOV_cjs.backgroundOverrideJsonSchema;
  ...
  getParameters = () => {
    if (chunkY4FZMBP5_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkY4FZMBP5_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkY4FZMBP5_cjs.isVercelTool(tool)) {
          result = await chunkLP4WZA6D_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkFHPG32XN_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            ...chunkFHPG32XN_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkY4FZMBP5_cjs.validateToolInput(resumeSchema, resumeData, options.name);
        ...
        if (suspendData) {
          ...
          const suspendValidation = chunkY4FZMBP5_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkY4FZMBP5_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkY4FZMBP5_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      if (fgaProvider) {
        const { getAgentToolFGAResourceId, getMCPToolFGAResourceId, getStandaloneToolFGAResourceId, requireFGA } = await import('./fga-check-JNZ754RN.cjs');
        ...
        await requireFGA({
          ...
          permission: chunkGBRVBJXF_cjs.MastraFGAPermissions.TOOLS_EXECUTE,
          requestContext: toolRequestContext,
          actor: execOptions?.actor,
          context: {
            resourceId: options.resourceId
      ...
      try {
        ...
        const { data, error } = chunkY4FZMBP5_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunkY4FZMBP5_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    const instanceNeedsApprovalFn = chunkY4FZMBP5_cjs.getNeedsApprovalFn(this.originalTool);
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkY4FZMBP5_cjs.Tool) && !chunkY4FZMBP5_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-3KZ2OT53.cjs.map
//# sourceMappingURL=chunk-3KZ2OT53.cjs.map