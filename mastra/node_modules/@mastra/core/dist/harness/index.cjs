...
function createSubagentTool(opts) {
  ...
  return chunkY4FZMBP5_cjs.createTool({
    id: "subagent",
...
${FORKED_SUBAGENT_TASK_NOTICE}`;
        ...
        if (context?.requestContext) {
          subagentRequestContext = new chunkPJIAL3WK_cjs.RequestContext(context.requestContext.entries());
          if (harnessCtx) {
        ...
        subagentToRun = new chunk6ZBUYPOD_cjs.Agent({
        ...
        const allWorkspaceToolNames = workspace && allowedWs ? new Set(
          Object.keys(
            await chunk6ZBUYPOD_cjs.createWorkspaceTools(workspace, {
...
function patchTaskToolForFork(tool, toolId) {
  const stubExecute = async () => {
    ...
    if (toolId === "task_check") {
      const taskCheck = chunk6ZBUYPOD_cjs.summarizeTaskCheck([]);
      return {
        ...baseResult,
...
var FABLE_FALLBACK_MODEL = "claude-opus-4-8";
var HARNESS_MAX_STEPS = 1e3;
...
function describeServerSideFallback(providerMetadata) {
  const fallback = chunk6ZBUYPOD_cjs.getServerSideFallbackInfo(providerMetadata);
  if (!fallback) {
    return void 0;
  }
...
function getDisplayTransform(metadata, phase, fallback) {
  const transform = chunk2TATDSHU_cjs.getTransformedToolPayload(metadata, "display", phase);
  return chunk2TATDSHU_cjs.hasTransformedToolPayload(transform) ? transform.transformed : fallback;
}
...
function toUserSignalMessage(payload) {
  ...
  const signal = chunk2TATDSHU_cjs.createSignal({
    id,
    type: "user",
    tagName: "user",
...
var Harness = class {
  ...
  constructor(config) {
    ...
    if (config.workspace instanceof chunk6ZBUYPOD_cjs.Workspace) {
  ...
  async init() {
    if (this.config.storage) {
      ...
      this.#internalMastra = new chunk6ZBUYPOD_cjs.Mastra({
        logger: false,
    ...
    if (this.config.workspace && !this.workspaceInitialized && !this.workspaceFn) {
      try {
        if (!this.workspace) {
          this.workspace = new chunk6ZBUYPOD_cjs.Workspace(this.config.workspace);
        }
  ...
  getAgentForMode(mode) {
    ...
    if (!this.#legacyAgentMode[mode.id]) {
      ...
      this.#legacyAgentMode[mode.id] = new chunk6ZBUYPOD_cjs.Agent({
        id: `${this.id}-agent`,
  ...
  async getCurrentModelAuthStatus() {
    ...
    try {
      ...
      if (currentModel) {
        return {
          ...
          apiKeyEnvVar: currentModel.hasApiKey ? void 0 : currentModel.apiKeyEnvVar
        };
      }
    } catch {
    }
    ...
    if (this.config.modelAuthChecker && provider) {
      ...
      if (result !== void 0) return { hasAuth: result };
    }
    return { hasAuth: true };
  }
  /**
...
${fence}` };
  ...
  async buildAgentMessageStreamOptions({
    ...
    const modeInstructions = this.config.agent ? this.resolveCurrentModeInstructions() : void 0;
    const streamOptions = {
  ...
  convertToHarnessMessage(msg) {
    ...
    if (msg.role === "signal") {
      const signal = chunk2TATDSHU_cjs.mastraDBMessageToSignal(msg);
      if (signal.type === "user") {
  ...
  async processStreamChunk(state, chunk, requestContext) {
    ...
    switch (chunk.type) {
      ...
      case "tool-call-delta": {
        ...
        const transform = chunk2TATDSHU_cjs.getTransformedToolPayload(chunk.metadata, "display", "input-delta");
        if (!transform?.suppress) {
          this.emit({
            type: "tool_input_delta",
            toolCallId,
            argsTextDelta: chunk2TATDSHU_cjs.hasTransformedToolPayload(transform) ? transform.transformed : argsTextDelta,
            toolName
          });
        }
        break;
      ...
      case "tool-call-approval": {
        ...
        const approvalTransform = chunk2TATDSHU_cjs.getTransformedToolPayload(chunk.metadata, "display", "approval");
        const toolArgs = chunk2TATDSHU_cjs.hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", chunk.payload.args);
  ...
  resetThreadDisplayState() {
    ...
    this.displayState.pendingSuspensions = /* @__PURE__ */ new Map();
   ...
   */
  resolvePendingSuspensionToolCallId(toolCallId) {
    if (toolCallId) {
      return this.pendingSuspensions.has(toolCallId) ? toolCallId : void 0;
    }
   ...
   */
  async handlePlanApprovalResume({
    toolCallId,
    response,
    requestContext
  }) {
    ...
    const currentMode = this.getCurrentMode();
    const transitionModeId = currentMode.transitionsTo ?? this.config.defaultModeId ?? this.config.modes.find((mode) => mode.default || mode.metadata?.default === true)?.id ?? this.config.modes[0]?.id;
  ...
  applyDisplayStateUpdate(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : chunk3KZ2OT53_cjs.safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
  ...
  async buildToolsets(requestContext) {
    const builtInTools = {
      ask_user: chunk6ZBUYPOD_cjs.askUserTool,
      submit_plan: chunk6ZBUYPOD_cjs.submitPlanTool,
      task_write: chunk6ZBUYPOD_cjs.taskWriteTool,
      task_update: chunk6ZBUYPOD_cjs.taskUpdateTool,
      task_complete: chunk6ZBUYPOD_cjs.taskCompleteTool,
      task_check: chunk6ZBUYPOD_cjs.taskCheckTool
    };
    ...
    if (this.config.subagents?.length && this.config.resolveModel) {
      ...
      builtInTools.subagent = createSubagentTool({
        ...
        cloneThreadForFork: hasMemory ? async ({ sourceThreadId, resourceId, title }) => {
          const memory = await this.resolveMemory();
          const result2 = await memory.cloneThread({
            sourceThreadId,
            resourceId: resourceId ?? this.resourceId,
            title,
            metadata: {
          ...
          return { id: result2.thread.id, resourceId: result2.thread.resourceId };
        } : void 0,
    ...
    const result = { harnessBuiltIn: builtInTools };
  ...
  async buildRequestContext(requestContext) {
    requestContext ??= new chunkPJIAL3WK_cjs.RequestContext();
...
Object.defineProperty(exports, "askUserTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.askUserTool; }
});
Object.defineProperty(exports, "assignTaskIds", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.assignTaskIds; }
});
Object.defineProperty(exports, "submitPlanTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.submitPlanTool; }
});
Object.defineProperty(exports, "taskCheckTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.taskCheckTool; }
});
Object.defineProperty(exports, "taskCompleteTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.taskCompleteTool; }
});
Object.defineProperty(exports, "taskUpdateTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.taskUpdateTool; }
});
Object.defineProperty(exports, "taskWriteTool", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.taskWriteTool; }
});
