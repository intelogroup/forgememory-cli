import { Workspace, Mastra, Agent, taskCheckTool, taskCompleteTool, taskUpdateTool, taskWriteTool, submitPlanTool, askUserTool, getServerSideFallbackInfo, createWorkspaceTools, summarizeTaskCheck } from '../chunk-5YZI33XV.js';
export { askUserTool, assignTaskIds, submitPlanTool, taskCheckTool, taskCompleteTool, taskUpdateTool, taskWriteTool } from '../chunk-5YZI33XV.js';
import { safeStringify } from '../chunk-VA4S6KHU.js';
import { createTool } from '../chunk-LCWNYZTU.js';
import { toStandardSchema } from '../chunk-6SRTDZ7S.js';
import { createSignal, mastraDBMessageToSignal, getTransformedToolPayload, hasTransformedToolPayload } from '../chunk-RIEQKS4L.js';
...
${fence}` };
  ...
  convertToHarnessMessage(msg) {
    ...
    if (msg.role === "signal") {
      const signal = mastraDBMessageToSignal(msg);
      if (signal.type === "user") {
  ...
  applyDisplayStateUpdate(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
...
export { Harness, defaultDisplayState, defaultOMProgressState, parseSubagentMeta };
