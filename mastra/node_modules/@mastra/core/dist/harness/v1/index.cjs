...
var RESERVED_EVENT_TYPES = /* @__PURE__ */ new Set([
  "session_created",
  "mode_changed",
  "model_changed",
  "thread_cloned",
...
// src/harness/v1/request-context.ts
function buildHarnessRequestContext({
  harnessContext
}) {
...
// src/harness/v1/skills.types.ts
var HarnessSkillNotFoundError = class extends Error {
  name = "HarnessSkillNotFoundError";
  skillName;
  searchedSources;
  constructor(opts) {
...
function recoverableError(error, code = "harness.tool_failed") {
  return {
    isError: true,
    code,
...
async function replaceTasks(session, tasks) {
  return session.updateState(() => ({
    updates: { tasks: tasks.map((task) => ({ ...task })) },
    result: tasks.map((task) => ({ ...task }))
  }));
}
async function updateTask(session, taskId, updates) {
  return session.updateState((state) => {
    const tasks = (state.tasks ?? []).map((task2) => ({ ...task2 }));
    const task = tasks.find((task2) => task2.id === taskId);
    if (!task) {
...
function buildHarnessBuiltInTools(session) {
  return {
    task_write: chunkY4FZMBP5_cjs.createTool({
      id: "task_write",
      description: "Replace the task list for the current harness session.",
      inputSchema: zod.z.object({ tasks: zod.z.array(taskSchema).min(1) }),
      execute: async ({ tasks }, context) => {
        try {
          assertOwningSession(session, context);
          const savedTasks = await replaceTasks(
            session,
            tasks.map((task) => ({ ...task, id: task.id ?? crypto.randomUUID() }))
          );
    ...
    task_update: chunkY4FZMBP5_cjs.createTool({
      id: "task_update",
    ...
    task_complete: chunkY4FZMBP5_cjs.createTool({
      id: "task_complete",
      description: "Mark one task completed in the current harness session.",
      inputSchema: zod.z.object({ id: zod.z.string() }),
    ...
    ask_user: chunkY4FZMBP5_cjs.createTool({
      id: "ask_user",
      description: "Register a pending question for the current harness session.",
      inputSchema: zod.z.object({
        question: zod.z.string(),
        options: zod.z.array(zod.z.object({ label: zod.z.string(), description: zod.z.string().nullable().optional() })).min(2).max(4).optional().nullable(),
        selectionMode: zod.z.enum(["single_select", "multi_select"]).optional()
      }),
    ...
    submit_plan: chunkY4FZMBP5_cjs.createTool({
      id: "submit_plan",
      description: "Register a pending plan approval for the current harness session.",
      inputSchema: zod.z.object({ title: zod.z.string().nullable().optional(), plan: zod.z.string() }),
      execute: async (input, context) => {
        try {
          assertOwningSession(session, context);
          const pending = await session.registerPendingItem({
    ...
    subagent: chunkY4FZMBP5_cjs.createTool({
      id: "subagent",
      description: "Create a durable child harness session for a configured subagent type.",
      inputSchema: zod.z.object({
        agentType: zod.z.string(),
        prompt: zod.z.string(),
        modelId: zod.z.string().optional().nullable(),
        forked: zod.z.boolean().optional().nullable()
      }),
      execute: async (input, context) => {
        try {
          assertOwningSession(session, context);
          return await session.spawnSubagentSession({
            ...
            ...input.forked !== void 0 && input.forked !== null ? { forked: input.forked } : {}
          });
        } catch (error) {
          return recoverableError(error);
        }
    ...
    useSkill: chunkY4FZMBP5_cjs.createTool({
      id: "useSkill",
      description: "Activate a skill in the current harness session.",
      inputSchema: zod.z.object({ ref: zod.z.string() }),
...
function buildSessionToolsets(opts = {}) {
  const { agentTools, modeOverrides, builtInTools, permissionRules, disabledTools } = opts;
  const base = modeOverrides?.tools ? { ...modeOverrides.tools } : { ...agentTools };
  ...
  const denySet = new Set(disabledTools ?? []);
...
var Session = class _Session {
  ...
  constructor(config) {
    ...
    this.#source = config.record?.source ? { type: config.record.source.type, parentSessionId: config.record.source.parentSessionId } : { type: config.record?.origin ?? "top-level", parentSessionId: config.record?.parentSessionId };
  ...
  async registerPendingItem(item) {
    const now = /* @__PURE__ */ new Date();
    const record = {
      ...item,
      sessionId: this.#id,
  ...
  async clone(opts = {}) {
    ...
    const cloneId = opts.sessionId ?? crypto.randomUUID();
  ...
  async signal({ messages, ...options }) {
    ...
    try {
      ...
      const model = this.#resolveModel ? await this.#resolveModel(this.#modelId) : void 0;
      const result = await agent.generate(messages, {
        ...
        activeTools: options.activeTools ? [.../* @__PURE__ */ new Set([...options.activeTools, ...Object.keys(tools)])] : Object.keys(tools)
      });
      this.#markIdle();
      return result;
    } catch (error) {
  ...
  async getSkill(name) {
    ...
    if (!workspaceSkills.some((skill2) => skill2.name === name)) return null;
   ...
   */
  refreshSkills() {
    this.#workspaceSkillsPromise = void 0;
  }
  #toHarnessSkill(skill) {
    const metadata = this.#plainMetadata(skill.metadata);
    const category = typeof metadata?.category === "string" ? metadata.category : void 0;
  ...
  #isJsonSerializable(value) {
    ...
    if (this.#isPlainObject(value))
      return Object.values(value).every((item) => item !== void 0 && this.#isJsonSerializable(item));
    return false;
  }
  #searchedSources() {
    return this.#workspace !== void 0 ? ["workspace"] : [];
  }
  async #loadWorkspaceSkillMetadata() {
  ...
  async #discoverWorkspaceSkillMetadata() {
    const workspace = await this.#getResolvedWorkspace();
  ...
  async #getResolvedWorkspace(requestContext) {
    const workspace = this.#workspace;
    if (!workspace) return void 0;
    if (typeof workspace !== "function") return workspace;
    if (this.#workspaceResolved) return this.#resolvedWorkspace;
    const resolved = await workspace({ requestContext: requestContext ?? new chunkPJIAL3WK_cjs.RequestContext() });
  ...
  async #applyStateUpdates(updates) {
    ...
    await this.#persistSession({ state: this.#state });
    this.#events.emit({
      type: "state_changed",
      state: this.#state,
      changedKeys
  ...
  #isBusySnapshot() {
    const hasActiveRun = this.#runStatus === "starting" || this.#runStatus === "running" || this.#runStatus === "waiting" || this.#runStatus === "resuming";
    return hasActiveRun || this.#pending.some((item) => item.status === "pending");
  }
  #markRunning(runId, traceId = null) {
  ...
  async #persistSession(updates) {
    this.#lastActivityAt = updates.lastActivityAt ?? /* @__PURE__ */ new Date();
    await this.#storage.updateSession(this.#id, {
      ...updates,
      lastActivityAt: this.#lastActivityAt
    });
  }
  async #respondToPendingItem(pendingItemId, expectedKind, response) {
    const item = this.#pending.find((item2) => item2.id === pendingItemId);
    if (!item) {
      throw new Error(`Harness pending item "${pendingItemId}" was not found on session "${this.#id}"`);
    }
    ...
    const recordedResponse = resumeResult === void 0 ? response : { ...response, resumeResult };
    await this.#storage.updatePendingItem(this.#id, pendingItemId, { status: "responded", response: recordedResponse });
    await this.#reloadRecordProjection();
    const updated = this.#pending.find((item2) => item2.id === pendingItemId);
    if (!updated) {
      throw new Error(`Harness pending item "${pendingItemId}" was not found on session "${this.#id}"`);
    }
    return { ...updated };
  }
  async #resumePendingBoundary(item, response) {
    const payload = item.payload;
    if (!this.#isPlainObject(payload)) return void 0;
    if (item.kind === "tool-approval") {
      ...
      const agent = typeof payload.agentId === "string" && this.#resolveAgent ? await this.#resolveAgent(payload.agentId) : this.#agent;
      if (typeof approved !== "boolean" || !runId || !this.#resolveAgent) return void 0;
      const result = approved ? await agent.approveToolCallGenerate({ runId, toolCallId, requestContext: await this.#buildRequestContext() }) : await agent.declineToolCallGenerate({ runId, toolCallId, requestContext: await this.#buildRequestContext() });
      return this.#isJsonSerializable(result) ? result : { resumed: true };
    }
    if (item.kind === "plan-approval" && response.approved === true) {
      const transitionModeId = typeof payload.transitionModeId === "string" ? payload.transitionModeId : void 0;
...
function isHarnessStorage(storage) {
  return typeof storage.loadSession === "function";
}
var Harness = class {
  #ownerId;
  #defaultMode;
  ...
  constructor(config) {
    ...
    this.#ownerId = config.ownerId ?? crypto.randomUUID();
    ...
    if (config.storage) {
      if (isHarnessStorage(config.storage)) {
        ...
        this.#compositeStorage = chunk6ZBUYPOD_cjs.augmentWithInit(config.storage);
      }
    } else {
    ...
    if (config.runtimeCompatibilityGeneration !== void 0 && config.runtimeCompatibilityGeneration.length === 0) {
    ...
    if (config.workspace instanceof chunk6ZBUYPOD_cjs.Workspace || typeof config.workspace === "function") {
      this.#workspace = config.workspace;
    } else if (config.workspace) {
      this.#workspace = new chunk6ZBUYPOD_cjs.Workspace(config.workspace);
    }
    this.#agent = this.#mastra ? this.#mastra.getAgentById(config.agent) : config.agent;
    if (config.subagents) {
    ...
    if (config.toolCategoryResolver) {
      ...
      this.#toolCategoryResolver = (toolName) => categories[toolName] ?? null;
    }
  ...
  #sessionFromRecord(record, storage) {
  ...
  #resolveAgent(agentId) {
    if (this.#mastra) return this.#mastra.getAgentById(agentId);
  ...
  #sessionIdFor(resourceId, threadId) {
    const hash = crypto.createHash("sha256").update(`${resourceId}\0${threadId}`).digest("hex").slice(0, 32);
    return `sess-${hash}`;
  }
};
...
exports.HarnessSkillNotFoundError = HarnessSkillNotFoundError;
exports.Session = Session;
exports.formatHarnessEventId = formatHarnessEventId;
exports.parseHarnessEventId = parseHarnessEventId;
