...
chunkZFOCQ6NW_cjs.createProviderDefinedToolFactoryWithOutputSchema({
...
var fileSearch = chunkZFOCQ6NW_cjs.createProviderDefinedToolFactoryWithOutputSchema({
...
// ../../../../../setup-pnpm/node_modules/.bin/store/v11/links/@ai-sdk/azure/2.0.108/b619578b5433f96e73335d3c59b38f31d90988cae8d0d17a40a0e35a5de905d1/node_modules/@ai-sdk/azure/dist/index.mjs
...
function createAzure(options = {}) {
  ...
  const getHeaders = () => {
    const baseHeaders = {
      "api-key": chunkZFOCQ6NW_cjs.loadApiKey({
    ...
    return chunkZFOCQ6NW_cjs.withUserAgentSuffix(baseHeaders, `ai-sdk/azure/${VERSION}`);
  };
  const getResourceName = () => chunkZFOCQ6NW_cjs.loadSetting({
  ...
  const url = ({ path: path2, modelId }) => {
    var _a2;
    const baseUrlPrefix = (_a2 = options.baseURL) != null ? _a2 : `https://${getResourceName()}.openai.azure.com/openai`;
    let fullUrl;
    if (options.useDeploymentBasedUrls) {
      fullUrl = new URL(`${baseUrlPrefix}/deployments/${modelId}${path2}`);
    } else {
      fullUrl = new URL(`${baseUrlPrefix}/v1${path2}`);
    }
...
createAzure();
var TERMINAL_RESPONSE_EVENTS = /* @__PURE__ */ new Set(["response.completed", "response.failed", "response.incomplete", "error"]);
function createOpenAIWebSocketFetch(options) {
  ...
  function getConnection(authorization, headers, signal) {
    ...
    if (ws?.readyState === WebSocket__default.default.OPEN && connectionKey === nextConnectionKey) {
    ...
    connecting = new Promise((resolve, reject) => {
      let settled = false;
      const socket = new WebSocket__default.default(getWebSocketUrl(wsUrl, apiKeyQueryParam, apiKey), {
        headers: {
          ...normalizedHeaders,
...
function mirrorAzureResponseProviderOptions(prompt) {
  ...
  const mirroredPrompt = prompt.map((message) => {
    ...
    const content = message.content.map((part) => {
      ...
      const providerOptions2 = mirrorAzureProviderOptionsForOpenAI(part.providerOptions);
      if (providerOptions2 === part.providerOptions) {
        return part;
      }
      contentModified = true;
      return { ...part, providerOptions: providerOptions2 };
    });
    if (!contentModified) {
      if (providerOptionsModified) {
        promptModified = true;
  ...
  return new Proxy(model, {
    get(target, property, receiver) {
      if (property === "doGenerate") {
        return (options) => target.doGenerate({
          ...options,
      ...
      if (property === "doStream") {
        return (options) => target.doStream({
          ...options,
          providerOptions: mirrorAzureProviderOptionsForOpenAI(options.providerOptions),
          prompt: mirrorAzureResponseProviderOptions(options.prompt)
        });
...
var AzureOpenAIGateway = class extends chunkZFOCQ6NW_cjs.MastraModelGateway {
  constructor(config) {
    super();
    this.config = config;
    this.validateConfig();
  ...
  validateConfig() {
    ...
    if (this.config.useDeploymentBasedUrls === false && this.config.apiVersion && !["v1", "preview"].includes(this.config.apiVersion)) {
      throw new chunkXSOONORA_cjs.MastraError({
        id: "AZURE_GATEWAY_INVALID_CONFIG",
        domain: "LLM",
        category: "UNKNOWN",
  ...
  createAzureResponsesWebSocketFetch({
    useEntraId,
    responsesWebSocket
  }) {
  ...
  async resolveLanguageModel({
    ...
    const websocketFetch = useWebSocket ? this.createAzureResponsesWebSocketFetch({
      useEntraId,
      responsesWebSocket
    }) : void 0;
    const azureConfig = {
      ...
      headers: { "User-Agent": chunkZFOCQ6NW_cjs.MASTRA_USER_AGENT, ...headers },
    ...
    if (useResponsesAPI) {
      ...
      if (websocketFetch) {
        Object.defineProperty(model, chunkZFOCQ6NW_cjs.MASTRA_GATEWAY_STREAM_TRANSPORT, {
          configurable: true,
          value: {
...
var MastraGateway = class extends chunkZFOCQ6NW_cjs.MastraModelGateway {
  constructor(config) {
    super();
    this.config = config;
  }
  ...
  resolveLanguageModel({
    ...
    if (this.config?.customFetch && providerId === "anthropic") {
      return chunkZFOCQ6NW_cjs.createAnthropic({
        apiKey: "oauth-gateway-placeholder",
        baseURL,
        headers: {
          "User-Agent": chunkZFOCQ6NW_cjs.MASTRA_USER_AGENT,
          [chunkZFOCQ6NW_cjs.GATEWAY_AUTH_HEADER]: `Bearer ${apiKey}`,
          ...headers
        },
    ...
    if (this.config?.customFetch) {
      return chunkDK36PFIR_cjs.createOpenRouter({
        apiKey: "oauth-gateway-placeholder",
        baseURL,
        headers: {
          "User-Agent": chunkZFOCQ6NW_cjs.MASTRA_USER_AGENT,
...
function shouldEnableGateway(gateway) {
...
function getEnabledGatewayIds(gateways) {
  const enabledGatewayIds = /* @__PURE__ */ new Set();
...
function sanitizeRegistryDataForRuntime(data, enabledGatewayIds) {
...
var GLOBAL_CAPABILITIES_DIR = () => path__default.default.join(CACHE_DIR(), "capabilities");
...
function getPackageRoot() {
  try {
    const require2 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-RWB3UEF4.cjs', document.baseURI).href)) || "file://");
...
function loadRegistry(useDynamicLoading, customGateways = []) {
  const enabledGatewayIds = getEnabledGatewayIds([
    new chunkDK36PFIR_cjs.ModelsDevGateway({}),
    new chunk6W5DBWIM_cjs.NetlifyGateway(),
    new MastraGateway(),
    ...customGateways
  ]);
  if (!useDynamicLoading) {
    return sanitizeRegistryDataForRuntime(provider_registry_default, enabledGatewayIds);
  }
  ...
  for (const jsonPath of possiblePaths) {
    try {
      ...
      if (!isFileNotFound && isJsonParseError) {
        ...
        registryData = sanitizeRegistryDataForRuntime(provider_registry_default, enabledGatewayIds);
        return registryData;
      }
      continue;
    }
...
var providerCapCache = /* @__PURE__ */ new Map();
function isDirectory(dir) {
  try {
    return fs__default.default.existsSync(dir) && fs__default.default.statSync(dir).isDirectory();
  } catch {
    return false;
  }
}
function findCapabilitiesDirs(useDynamicLoading) {
  const packageRoot = getPackageRoot();
  const distCapabilitiesDir = path__default.default.join(packageRoot, "dist", "capabilities");
  const sourceCapabilitiesDir = path__default.default.join(packageRoot, "src", "llm", "model", "capabilities");
  const workspaceSourceCapabilitiesDir = path__default.default.join(process.cwd(), "packages/core/src/llm/model/capabilities");
  const dirs = [];
  if (useDynamicLoading) {
...
var capabilitiesDirCache;
function loadProviderAttachmentModels(provider, useDynamicLoading) {
  if (providerCapCache.has(provider)) return providerCapCache.get(provider);
  if (capabilitiesDirCache === void 0) {
    capabilitiesDirCache = findCapabilitiesDirs(useDynamicLoading);
  }
  for (const capabilitiesDir of capabilitiesDirCache) {
    const filePath = path__default.default.join(capabilitiesDir, `${provider}.json`);
    try {
...
function getProviderAttachmentSupport(provider, modelId, useDynamicLoading) {
  const models = loadProviderAttachmentModels(provider, useDynamicLoading);
  if (!models) return void 0;
  return models.includes(modelId);
}
function modelSupportsAttachments(modelRouterId) {
  ...
  if (directSupport !== void 0) return directSupport;
  ...
  if (nestedProviderDelimiter !== -1) {
    ...
    if (nestedProvider && nestedModelId) {
      ...
      if (nestedSupport !== void 0) return nestedSupport;
    }
  }
...
var GatewayRegistry = class _GatewayRegistry {
  ...
  static getInstance(options) {
    ...
    if (options?.useDynamicLoading === true) {
      _GatewayRegistry.instance.useDynamicLoading = true;
    }
    return _GatewayRegistry.instance;
  }
  /**
  ...
  async syncGateways(forceRefresh = false, writeToSrc = false) {
    ...
    try {
      const { ModelsDevGateway: ModelsDevGateway2 } = await import('./models-dev-D63YLCGB.cjs');
      const { NetlifyGateway: NetlifyGateway2 } = await import('./netlify-PS3LK2HU.cjs');
      const { MastraGateway: MastraGateway2 } = await import('./mastra-GTKNCKZJ.cjs');
      const { fetchProvidersFromGateways, writeRegistryFiles } = await import('./registry-generator-DKLH6OH7.cjs');
      const defaultGateways = [
        new ModelsDevGateway2({}),
        new NetlifyGateway2(),
        ...writeToSrc ? [] : [new MastraGateway2()]
      ];
      ...
      if (shouldWriteToSrc) {
        ...
        const distCapDir = path__default.default.join(packageRoot, "dist", "capabilities");
        const srcCapDir = path__default.default.join(packageRoot, "src", "llm", "model", "capabilities");
        if (fs__default.default.existsSync(distCapDir)) {
          await fs__default.default.promises.mkdir(srcCapDir, { recursive: true });
          const capFiles = fs__default.default.readdirSync(distCapDir).filter((f) => f.endsWith(".json"));
          for (const file of capFiles) {
            await fs__default.default.promises.copyFile(path__default.default.join(distCapDir, file), path__default.default.join(srcCapDir, file));
          }
        }
      }
...
var autoRefreshEnabled = !isOfflineMode() && (process.env.MASTRA_AUTO_REFRESH_PROVIDERS === "true" || process.env.MASTRA_AUTO_REFRESH_PROVIDERS !== "false" && isDev);
...
exports.MastraGateway = MastraGateway;
exports.PROVIDER_MODELS = PROVIDER_MODELS;
exports.PROVIDER_REGISTRY = PROVIDER_REGISTRY;
exports.createOpenAIWebSocketFetch = createOpenAIWebSocketFetch;
exports.findGatewayForModel = findGatewayForModel;
...
exports.modelSupportsAttachments = modelSupportsAttachments;
exports.parseModelString = parseModelString;
exports.shouldEnableGateway = shouldEnableGateway;
//# sourceMappingURL=chunk-RWB3UEF4.cjs.map
//# sourceMappingURL=chunk-RWB3UEF4.cjs.map