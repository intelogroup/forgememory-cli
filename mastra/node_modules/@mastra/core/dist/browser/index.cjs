...
var BROWSER_PROCESS_ID = crypto.randomUUID();
var BrowserContextProcessor = class {
  ...
  async computeStateSignal(args) {
    ...
    if (previousState?.open && browserState.open && previousState.activeUrl && browserState.activeUrl && previousState.activeUrl !== browserState.activeUrl && previousState.processId === browserState.processId && !browserState.activeUrlChangeSource) {
...
var MastraBrowser = class _MastraBrowser extends chunkWSD4JNMB_cjs.MastraBase {
  /**
  ...
  // ---------------------------------------------------------------------------
  constructor(config = {}) {
    super({ name: "MastraBrowser", component: chunkYNLHR2WT_cjs.RegisteredLogger.BROWSER });
    this.config = config;
...
2. Remove profile/executablePath to connect to the existing browser via CDP`
  ...
  getInputProcessors(configuredProcessors = []) {
    const hasProcessor = configuredProcessors.some(
      (p) => !chunkUHTO5JDE_cjs.isProcessorWorkflow(p) && "id" in p && p.id === "browser-context"
    );
...
var FOURCC_RIFF = fourcc("RIFF");
var FOURCC_AVI = fourcc("AVI ");
var FOURCC_LIST = fourcc("LIST");
var FOURCC_HDRL = fourcc("hdrl");
var FOURCC_AVIH = fourcc("avih");
var FOURCC_STRL = fourcc("strl");
var FOURCC_STRH = fourcc("strh");
var FOURCC_STRF = fourcc("strf");
var FOURCC_VIDS = fourcc("vids");
var FOURCC_MJPG = fourcc("MJPG");
var FOURCC_MOVI = fourcc("movi");
var FOURCC_IDX1 = fourcc("idx1");
var FOURCC_00DC = fourcc("00dc");
var AVIF_HASINDEX = 16;
var AVIIF_KEYFRAME = 16;
var AVIH_SIZE = 56;
var STRH_SIZE = 56;
var STRF_SIZE = 40;
var LIST_TYPE_SIZE = 4;
...
function writeMjpegAviFile(filePath, frames, opts) {
  ...
  const riffPayloadSize = 4 + CHUNK_HEADER_SIZE + hdrl.length + CHUNK_HEADER_SIZE + moviPayloadSize + CHUNK_HEADER_SIZE + idx1PayloadSize;
...
function selectCaptionAt(captions, t) {
  ...
  for (const c of captions) {
    if (t >= c.timestampMs && t < c.timestampMs + c.durationMs) {
      if (!active2 || c.timestampMs > active2.timestampMs) {
        active2 = c;
      }
    }
  }
...
function decodeJpeg(jpegBytes) {
  const decoded = jpegJs.decode(jpegBytes, { useTArray: true, formatAsRGBA: true });
  return {
    width: decoded.width,
...
// src/browser/recording/tools.ts
var DEFAULT_MAX_DURATION_MS = 3e4;
var HARD_CAP_MAX_DURATION_MS = 12e4;
var DEFAULT_EVERY_NTH_FRAME = 2;
var DEFAULT_MAX_WIDTH = 1024;
var DEFAULT_MAX_HEIGHT = 720;
var MAX_CAPTION_LENGTH = 80;
var DEFAULT_CAPTION_DURATION_MS = 2500;
var MIN_CAPTION_SPACING_MS = 500;
var MAX_BUFFERED_FRAMES = 2e3;
var FRAME_STALL_MS = 4e3;
...
function defaultOutputPath(id, outputDir) {
  return path.join(recordingsDir(outputDir), `${id}.avi`);
}
function resolveOutputPath(id, outputDir, requestedPath) {
  const baseDir = recordingsDir(outputDir);
  const outputPath = requestedPath ? path.resolve(requestedPath) : defaultOutputPath(id, outputDir);
  const rel = path.relative(baseDir, outputPath);
...
async function startRecording(browser, opts) {
  ...
  const maxDurationMs = Math.min(
    ...
    Math.max(1e3, opts.maxDurationMs ?? DEFAULT_MAX_DURATION_MS)
  );
...
var recordSchema = zod.z.object({
  action: zod.z.enum(["start", "stop", "status"]).describe(
  ...
  outputPath: zod.z.string().min(1).optional().describe(
...
var recordCaptionSchema = zod.z.object({
...
function createBrowserRecordingTools(browser, options) {
  ...
  return {
    browser_record: chunkY4FZMBP5_cjs.createTool({
      id: "browser_record",
    ...
    browser_record_caption: chunkY4FZMBP5_cjs.createTool({
...
Object.defineProperty(exports, "BrowserCliHandler", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.BrowserCliHandler; }
});
Object.defineProperty(exports, "browserCliHandler", {
  enumerable: true,
  get: function () { return chunk6ZBUYPOD_cjs.browserCliHandler; }
