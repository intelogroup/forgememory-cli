export { BrowserCliHandler, browserCliHandler } from '../chunk-5YZI33XV.js';
import { isProcessorWorkflow } from '../chunk-X4ZG5TF4.js';
import { createTool } from '../chunk-LCWNYZTU.js';
import { MastraBase } from '../chunk-77VL4DNS.js';
import { RegisteredLogger } from '../chunk-O274XICP.js';
import { existsSync, readdirSync, lstatSync, unlinkSync, mkdirSync, openSync, closeSync, writeSync, fstatSync } from 'fs';
...
var BROWSER_PROCESS_ID = randomUUID();
...
var MastraBrowser = class _MastraBrowser extends MastraBase {
  /**
...
var ScreencastStream = class extends EventEmitter {
...
function decodeJpeg(jpegBytes) {
  const decoded = decode(jpegBytes, { useTArray: true, formatAsRGBA: true });
  return {
    width: decoded.width,
...
var recordSchema = z.object({
  ...
  maxDurationMs: z.number().int().positive().optional().describe('Only used with action="start". Maximum recording duration in ms (default 30000, hard cap 120000).'),
  everyNthFrame: z.number().int().positive().optional().describe('Only used with action="start". Capture every Nth screencast frame (default 2). Higher = smaller file.'),
  maxWidth: z.number().int().positive().optional().describe('Only used with action="start". Max frame width in pixels (default 1024).'),
  maxHeight: z.number().int().positive().optional().describe('Only used with action="start". Max frame height in pixels (default 720).'),
  outputPath: z.string().min(1).optional().describe(
...
var recordCaptionSchema = z.object({
  text: z.string().min(1).max(MAX_CAPTION_LENGTH).describe("Very short caption (max ~6 words) describing what you just did in the browser."),
  durationMs: z.number().int().positive().optional().describe("How long the caption should remain visible in the video (ms). Default 2500ms.")
});
...
export { BrowserContextProcessor, DEFAULT_THREAD_ID, MastraBrowser, SCREENCAST_DEFAULTS, ScreencastStream as ScreencastStreamImpl, ThreadManager, createBrowserRecordingTools, createError };
