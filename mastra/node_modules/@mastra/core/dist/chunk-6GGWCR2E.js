import { InMemoryDB, InMemoryAgentsStorage, FilesystemAgentsStorage } from './chunk-PRF2LRAU.js';
import { InMemorySkillsStorage, FilesystemSkillsStorage } from './chunk-CB3IKS2K.js';
import { InMemoryFavoritesStorage } from './chunk-7QYTAPIQ.js';
import { InMemoryMCPClientsStorage, FilesystemMCPClientsStorage } from './chunk-KFGBLAYE.js';
import { InMemoryMCPServersStorage, FilesystemMCPServersStorage } from './chunk-NOSHI7SE.js';
import { InMemoryPromptBlocksStorage, FilesystemPromptBlocksStorage } from './chunk-SETBJRQH.js';
import { InMemoryScorerDefinitionsStorage, FilesystemScorerDefinitionsStorage } from './chunk-XCALE2SP.js';
import { InMemoryWorkspacesStorage, FilesystemWorkspacesStorage } from './chunk-MEB7F5BR.js';
import { StorageDomain, normalizePerPage, calculatePagination, MastraCompositeStore, InMemoryThreadStateStorage } from './chunk-DJYI3UM2.js';
import { getBranchArgsSchema, extractBranchSpans, TABLE_SCHEMAS, TABLE_SCORERS, BRANCH_SPAN_TYPE_SET, listTracesArgsSchema, toTraceSpans, listBranchesArgsSchema, toTraceSpan, TABLE_NOTIFICATIONS, TABLE_WORKFLOW_SNAPSHOT } from './chunk-WNZ2RDFB.js';
import { parseMemoryRequestContext, applyStateSignal } from './chunk-OZHPE46K.js';
import { EventEmitterPubSub } from './chunk-SFA7SIBL.js';
import { coreFeatures } from './chunk-GAHS7ZSL.js';
import { EntityType, listMetricsArgsSchema, listLogsArgsSchema, listScoresArgsSchema, listFeedbackArgsSchema } from './chunk-DEQCJWZZ.js';
import { MastraBase } from './chunk-77VL4DNS.js';
import { MessageList, createMessageSignal, createSignal, resolveDeliveryAttributes } from './chunk-RIEQKS4L.js';
import { MastraError, ErrorCategory, ErrorDomain } from './chunk-M7RBQNFP.js';
import { MASTRA_THREAD_ID_KEY, MASTRA_RESOURCE_ID_KEY } from './chunk-RI37F2KY.js';
import { jsonSchemaToZod } from '@mastra/schema-compat/json-to-zod';
import { z } from 'zod/v4';
import { randomUUID } from 'crypto';
import { join, dirname, resolve, sep, extname, relative } from 'path';
import { existsSync, mkdirSync, readFileSync, writeFileSync, renameSync, statSync, readdirSync, rmSync } from 'fs';

// src/storage/domains/observability/base.ts
var ObservabilityStorage = class _ObservabilityStorage extends StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "OBSERVABILITY"
  ...
  async getBranch(args) {
    ...
    try {
      ...
      const isFallbackTrigger = error instanceof MastraError && (error.id === "OBSERVABILITY_STORAGE_GET_STRUCTURE_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_TRACE_LIGHT_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_SPANS_NOT_IMPLEMENTED");
...
var defaultAgentThreadPubSub = new EventEmitterPubSub();
...
var AgentThreadStreamRuntime = class {
  ...
  #getThreadTarget(options) {
    ...
    const threadId = options?.requestContext?.get(MASTRA_THREAD_ID_KEY) || (typeof thread === "string" ? thread : thread?.id);
    const resourceId = options?.requestContext?.get(MASTRA_RESOURCE_ID_KEY) || options?.memory?.resource;
  ...
  sendMessage(agent, message, target, pubsub) {
    return this.sendSignal(agent, createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() }), target, pubsub);
  }
  queueMessage(agent, message, target, pubsub) {
    const state = this.#getState(pubsub);
    const signal = createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() });
    let key;
  ...
  sendSignal(agent, signalInput, target, pubsub) {
    const state = this.#getState(pubsub);
    let signal = createSignal({ ...signalInput, acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    let runId = target.runId;
...
export { BackgroundTasksInMemory, BackgroundTasksStorage, BlobStore, ChannelsStorage, DatasetsInMemory, DatasetsStorage, ExperimentsInMemory, ExperimentsStorage, FilesystemDB, FilesystemStore, GitHubSourceControlProvider, HarnessStorage, InMemoryBlobStore, InMemoryChannelsStorage, InMemoryHarness, InMemoryMemory, InMemoryNotificationsStorage, InMemorySchedulesStorage, InMemoryStore, InMemoryToolProviderConnectionsStorage, MemoryStorage, MockStore, NotificationsStorage, ObservabilityInMemory, ObservabilityStorage, SchedulesStorage, SchemaUpdateValidationError, SchemaValidationError, SchemaValidator, ScoresInMemory, ScoresStorage, StoreOperations, StoreOperationsInMemory, ToolProviderConnectionsStorage, WorkflowsInMemory, WorkflowsStorage, agentThreadStreamRuntime, buildCreateSpanRecord, buildFeedbackRecord, buildLogRecord, buildMetricRecord, buildScoreRecord, buildUpdateSpanRecord, cloneRunData, createEmptyWorkflowSnapshot, createGitHubSourceControlProviderFromEnv, createNotificationSignal, createNotificationSummarySignal, createStorageErrorId, createStoreErrorId, createValidator, createVectorErrorId, dispatchDueNotifications, ensureDate, filterByDateRange, getDefaultValue, getObjectOrNull, getSchemaValidator, getSqlType, getStringOrNull, jsonValueEquals, mergeWorkflowStepResult, notificationSignalAttributes, notificationSignalMetadata, notificationSummaryContents, notificationSummarySignalMetadata, safelyParseJSON, serializeDate, serializeSpanAttributes, summarizeNotifications, toEntityType, transformRow, transformScoreRow };
//# sourceMappingURL=chunk-6GGWCR2E.js.map
//# sourceMappingURL=chunk-6GGWCR2E.js.map